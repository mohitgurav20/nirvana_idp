...
# ✨ THE CRUCIAL FIX: Automatically clean up any blank lines or missing values (NaN)
data = data.dropna()

# 3. Validation Check: Make sure we have enough data points to train a line
if len(data) < 2:
    print("⚠️ Warning: Not enough rows in data.csv to train a model. Need at least 2 entries.")
    exit()
...
try:
    ...
    model.fit(X, y)
    print("🧠 Linear Regression Model trained successfully on your dataset!")
